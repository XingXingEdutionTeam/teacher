...
#define showSetPayPwdNil         @"请输入支付密码"
#define showSetPayPwdError       @"输入密码错误"
#define showSetPayPwdSuccess     @"输入密码成功"
...
#define cirImageWH 16      // 黑色图片的宽高


#define keyBoardH   200           //键盘的高度
#define keyBoardButtonDistance 1 // 键盘按钮边距
...
//        [cirBGView setBackgroundColor:arcColor]; // 测试的随机颜色
        ...
        pwdImage.layer.cornerRadius = cirImageWH / 2;
        pwdImage.layer.masksToBounds = YES;
...
//    sureButton.sd_layout.leftSpaceToView(cancelButton,pwdDistance).rightSpaceToView(showView,0).topSpaceToView(pwdView,0).bottomSpaceToView(showView,0);
    ...
    [sureButton setTitleColor:[UIColor blackColor] forState:UIControlStateHighlighted];
...
//    keyboardView.sd_layout.leftSpaceToView(bg,0).rightSpaceToView(bg,0).bottomSpaceToView(bg,0).heightIs(keyBoardH);
    [keyboardView setBackgroundColor:XXEBackgroundColor];
...
//    keyboardView.layer.masksToBounds=YES;
    ...
    arrKeyboardNum=@[@"1",@"2",@"3",@"4",@"5",@"6",@"7",@"8",@"9",@"0",@"清空",@"删除"]; //题外话:这里用的是大写的字母 X 并不是数学中的乘以符号x
    for(int i=0;i<arrKeyboardNum.count;i++){
        ...
        keyboardBtn.titleLabel.font = [UIFont systemFontOfSize:20];
...
-(void)addPwdImage{
    for (int i=0; i<self.arrAddPwd.count; i++) {
        ...
        if (i<clickNum) {
            imageView.image=[UIImage imageNamed:@"mimaicon"];
        }
        else{
            imageView.image=nil;
        }
