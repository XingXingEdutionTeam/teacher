...
- (void)changeMenuDataWithIndex:(NSInteger)index{
    ...
    if ([self.allDataSource[index] count] != 0) {
        arr = self.allDataSource[index];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.tableFirst) {
        ...
        cell1.textLabel.frame = CGRectMake(0, 0, cell1.size.width, cell1.size.height);
        cell1.textLabel.textAlignment = NSTextAlignmentCenter;
        
//        NSLog(@"cell1 ===%@", NSStringFromCGRect(cell1.textLabel.frame));
        ...
        if (cell2 == nil) {
            cell2 = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIde];
        }
        if (!self.data) {
        ...
        if (cell3 == nil) {
            cell3 = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIde];
        }
