...
//    XXEFlowerbasketApi *flowerbasketApi = [[XXEFlowerbasketApi alloc] initWithUrlString:URL appkey:APPKEY backtype:BACKTYPE xid:XID user_id:USER_ID user_type:USER_TYPE page:@"1"];
...
//    }];
    ...
    //路径
    NSString *urlStr = @"http://www.xingxingedu.cn/Teacher/give_fbasket_record";
    ...
    [XXEHttpTool post:urlStr params:params success:^(id responseObj) {
        //
//        NSLog(@"111  %@", responseObj);
        NSString *codeStr = [NSString stringWithFormat:@"%@", responseObj[@"code"]];
        
        if ([codeStr isEqualToString:@"1"]) {
            NSArray *modelArray = [XXEFlowerbasketModel parseResondsData:responseObj[@"data"]];
